...
), displayModeBar = FALSE, showTips = FALSE, showAxisDragHandles = FALSE)
```

## Organization Attractiveness

One way to interpret an inventor's move from one organization to another is that the target
organization is more attractive. From this, we might derive a measure of organization attractiveness,
and we might see if different organizations are attractive to the different sex groups:
...
kable(organization_move_frequency[c(1:20, 1:20 + nrow(organization_move_frequency) - 20), ], digits = 3)
```
