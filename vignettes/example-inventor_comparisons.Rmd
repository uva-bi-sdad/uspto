...
The mechanical search returned `r prettyNum(nrow(search_mechanical), big.mark = ",")` results, and the pharmaceutical search
returned `r prettyNum(nrow(search_pharmaceutical), big.mark = ",")` results.
...
summaries <- vapply(split(applications, applications$search_set), function(d) {
  c(
    ...
    "Mean Number of Inventors" = mean(d$n_inventors),
...
kable(summaries, digits = 3, col.names = c("Mechanical", "Pharmaceutical", "Difference"))
```
...
kable(prediction_summaries, digits = 3)
```
