...
#' @importFrom httr POST add_headers status_code http_error content PUT
#' @importFrom jsonlite read_json fromJSON write_json
#' @export

download_peds <- function(searchText = "*:*", filters = list("*:*"), outFile = NULL, start = 0, minMatch = "100%", overwrite = FALSE,
                          waits = 20, wait = 10, endpoint = "https://ped.uspto.gov/api/queries/") {
  ...
  final <- normalizePath(paste0(if (is.null(outFile)) hash else outFile, ".json"), "/", FALSE)
  if (!overwrite && file.exists(final)) {
    stop(
      "output file (", basename(final), ") already exists; add overwrite = TRUE to overwrite it",
      call. = FALSE
    )
  }
  ...
  if (overwrite || !file.exists(bundle)) {
    ...
    message("Package requested for ", res$queryResults$searchResponse$response$numFound, " record(s): ", ID)
    retries <- 1
    while (jsonlite::read_json(paste0(endpoint, ID))$jobStatus != "COMPLETED") {
      ...
      retries <- retries + 1
      cat(paste0("\rwaiting for bundle (try: ", retries, ") ... "))
      Sys.sleep(wait)
    }
    cat("complete\n")
  ...
  message("writing final results: ", normalizePath(final, "/", FALSE))
...
#' @return list with entries for \code{numFound} (total office actions meeting the criteria),
#' \code{start}, and \code{docs} with and entry with each included office action.
#' @examples
#' \dontrun{
...
download_office_actions <- function(criteria = "*:*", outFile = NULL, start = 0, rows = 9999, overwrite = FALSE,
  ...
  message("Found ", res$numFound, " office actions for ", criteria, "; including ", res$start, ":", length(res$docs))
  dir.create(dirname(final), FALSE, TRUE)
