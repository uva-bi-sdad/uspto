...
download_patents <- function(ids = NULL, outFile = NULL, ids_type = "application", query = NULL,
                             ...
                             grant = FALSE, limit = FALSE, series = "US", verbose = FALSE) {
  if (!is.null(outFile) && file.exists(outFile)) {
    ...
    if (!is.null(res$error)) stop("request failed: ", res$status, ": ", res$error, call. = FALSE)
    if (!length(res$results)) stop("numbers were not found", call. = FALSE)
